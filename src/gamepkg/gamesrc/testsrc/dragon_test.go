...
func TestCreateDragon(t *testing.T) {
	var (
		game       = gamepkg.Game{}
		gotDragon  = gamepkg.Dragon{}
		wantDragon = gamepkg.Dragon{}
		weather    = gamepkg.Weather{}
	)
	game.GameId = 7569516
	...
	dragonJson := gamepkg.CreateDragon(game, weather)
	...
	status, _ := gamepkg.ResolveBattle(dragonString, game.GameId)
	if status != "Victory" {
